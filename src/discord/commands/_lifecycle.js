...
function resolveBot(interaction) {
  ...
  if (!sel) throw new Error('Chưa chọn bot. Dùng `/select-bot` trước hoặc cung cấp ID.');
  return sel;
}
  ...
  async execute(interaction) {
    ...
    try {
      ...
      await interaction.editReply({
        embeds: [successEmbed('Bot Started', `Bot \`${instance.record.username}\` đang kết nối...\nDùng \`/status-bot\` để theo dõi.`)],
      });
    } catch (err) {
...
const stopCmd = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      await interaction.editReply({
        embeds: [successEmbed('Bot Stopped', `Bot \`${instance.record.username}\` đã dừng.${force ? ' (force)' : ''}`)],
      });
    } catch (err) {
      await interaction.editReply({ embeds: [errorEmbed(err.message)] });
    }
...
const restartCmd = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      await interaction.editReply({
        embeds: [successEmbed('Bot Restarted', `Bot \`${instance.record.username}\` đã được khởi động lại.`)],
      });
    } catch (err) {
      await interaction.editReply({ embeds: [errorEmbed(err.message)] });
    }
