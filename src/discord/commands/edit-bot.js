...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('id').setDescription('Bot ID').setRequired(true))
    .addStringOption(o => o.setName('host').setDescription('Server IP mới').setRequired(false))
    .addIntegerOption(o => o.setName('port').setDescription('Port mới').setMinValue(1).setMaxValue(65535).setRequired(false))
    .addStringOption(o => o.setName('version').setDescription('Version mới (vd: 1.20.4)').setRequired(false))
    .addStringOption(o => o.setName('password').setDescription('Mật khẩu mới').setRequired(false))
  ...
  async execute(interaction) {
    ...
    if (ALIVE_STATES.has(instance.state)) {
      return interaction.editReply({
        embeds: [errorEmbed(`Bot đang ở trạng thái **${instance.state}**. Hãy dùng \`/stop\` trước khi chỉnh sửa.`)],
      });
    }
    ...
    if (Object.keys(patch).length === 0) {
      return interaction.editReply({ embeds: [errorEmbed('Không có thông tin nào được thay đổi.')] });
    }

    try {
      await BotManager.editBot(instance.id, patch, interaction.user.id);
      const changed = Object.keys(patch).filter(k => k !== 'password').join(', ');
      await interaction.editReply({
        embeds: [successEmbed('Bot Updated', `Đã cập nhật: \`${changed || 'password'}\`\nBot ID: \`${instance.id.slice(0, 8)}\``)],
      });
    } catch (err) {
