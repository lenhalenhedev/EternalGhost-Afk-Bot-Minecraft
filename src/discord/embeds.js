...
function buildStatusEmbed(instance) {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      `Server: \`${r.host}:${r.port}\`  •  Version: \`${r.version}\``
    )
    .addFields(
      ...
      {
        name: '❤️ HP',
        value: `${instance.health.toFixed(1)} / 20`,
        inline: true,
      },
      { name: '🍗 Food', value: `${instance.food} / 20`, inline: true },
      {
        name: '📍 Position',
        value: formatPos(instance.position),
        inline: true,
      },
      {
        name: '🔁 Reconnects',
        value: String(instance.reconnectAttempts),
        inline: true,
      },
      {
        name: '🤖 Auto-Reconnect',
        value: r.autoReconnect ? '✅' : '❌',
        inline: true,
      },
      {
        name: '🆔 Bot ID',
...
function buildStatsEmbed(stats, bots) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: '🧠 Heap Used',
        value: formatMB(stats.memHeapUsed),
        inline: true,
      },
      { name: '💾 RSS', value: formatMB(stats.memRSS), inline: true },
      {
        name: '📦 Est. /bot RAM',
        value: `~${stats.estimatedPerBotMB} MB`,
        inline: true,
      },
