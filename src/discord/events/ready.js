  ...
  async execute(client) {
    ...
    try {
      if (config.discord.guildId) {
        ...
        logger.info(
          `[Discord] Registered ${commands.length} guild commands to ${config.discord.guildId}`
        );
      } else {
        ...
        logger.info(
          `[Discord] Registered ${commands.length} global commands (may take up to 1h to propagate)`
        );
      }
    } catch (err) {
